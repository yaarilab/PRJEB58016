...
awk -F'\t' 'NR==FNR && NR>1 {umi[\$1]=\$2;} NR!=FNR {if(/UMI=/){split(\$0,a,"UMI=");split(a[2],b,"|"); sub(/\/[12]$/, "", b[1]); print a[1]"UMI="b[1];} else {print}}' pair_pass.umi.convert ${R1} > ${R1}_umi-pass.fastq
awk -F'\t' 'NR==FNR && NR>1 {umi[\$1]=\$2;} NR!=FNR {if(/UMI=/){split(\$0,a,"UMI=");split(a[2],b,"|"); sub(/\/[12]$/, "", b[1]); print a[1]"UMI="b[1]"|"b[2];} else {print}}' pair_pass.umi.convert ${R2} > ${R2}_umi-pass.fastq
