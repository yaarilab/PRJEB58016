...
output:
 set val(name), file("*_primers-pass.fast*")  into g38_11_reads0_g_71
...
awk -F'\t' 'NR==FNR && NR>1 {umi[\$1]=\$2;} NR!=FNR {if(/UMI=/){split(\$0,a,"${umi_field}=");split(a[2],b,"|");sub(/[/]/, "", b[1]);sub(/[/12]/, "", b[1]); print a[1]"UMI="b[1];} else {print}}' pair_pass.umi.convert ${R1} > ${R1}'_umi-pass.fastq'
awk -F'\t' 'NR==FNR && NR>1 {umi[\$1]=\$2;} NR!=FNR {if(/UMI=/){split(\$0,a,"${umi_field}=");split(a[2],b,"|");sub(/[/]/, "", b[1]);sub(/[/12]/, "", b[1]); print a[1]"UMI="b[1]"|"b[2] ;} else {print}}' pair_pass.umi.convert ${R2} > ${R2}'_umi-pass.fastq'
...
R1 = readArray[0]
R2 = readArray[1]
SUFFIX = "_primers-pass.fastq_umi-pass"

"""
#!/bin/bash

	PREV_SUFFIX=${SUFFIX}
    SUFFIX="_primers-pass.fastq_umi-pass_cluster-pass.fastq"
	TMP1_SUFFIX="_primers-pass.fastq_umi-pass_cluster-pass.fastq.ori.fastq"
    TMP2_SUFFIX="_primers-pass.fastq_umi-pass_cluster-pass.fastq.UMI_TO_CLUSTER.txt"
    SUFFIX_f="_primers-pass.fastq_umi-pass_cluster-pass.fastq_cluster-pass_pass.fastq"
